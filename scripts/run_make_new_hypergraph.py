...
def main() -> None:
    ...
    try:
        ...
        raise ImportError("Please install instructor: pip install instructor") from exc
    ...
    if args.total_threads == 1:
        merged_graph_list = sorted(
            glob.glob(str(data_dir_output_path / "*_integrated.pkl")),
            reverse=True,
            key=extract_idx,
        )
        current_merged_i = extract_idx(merged_graph_list[0]) if merged_graph_list else 0

    print(f"Thread: {args.thread_index}/{args.total_threads}")
    print(f"merge_every: {args.merge_every}")
        ...
        for attempt in range(1, total_attempts + 1):
            try:
                ...
                if attempt >= total_attempts:
                    break
                print(
                    f"[llm] request failed (attempt {attempt}/{total_attempts}): {exc!r}; "
    ...
    if merged_graph_list:
        try:
            with open(merged_graph_list[0], "rb") as handle:
    ...
    with torch.no_grad():
        for i, doc in enumerate(doc_list):
            ...
            graph_path = data_dir_path / f"{graph_root}.pkl"
            integrated_path = data_dir_output_path / f"{graph_root}_integrated.pkl"
            ...
            if not needs_generation:
                try:
                    with open(graph_path, "rb") as handle:
                        existing_graph = pickle.load(handle)
                    if existing_graph is None or not hasattr(existing_graph, "incidence_dict"):
                        print(f"[generation] Found invalid graph pickle, regenerating: {graph_path}")
                        os.remove(graph_path)
                        needs_generation = True
                except Exception:
                    print(f"[generation] Could not read graph pickle, regenerating: {graph_path}")
                    try:
                        os.remove(graph_path)
                    except Exception:
                        pass
            ...
            if os.path.exists(integrated_path):
                ...
                try:
                    with open(integrated_path, "rb") as handle:
                        G = pickle.load(handle)
                except Exception:
                    pass
                continue
            ...
            try:
                with open(graph_path, "rb") as handle:
                    H0 = pickle.load(handle)
                if H0 is None or not hasattr(H0, "incidence_dict"):
                    print(f"[merge] doc graph still invalid after generation: {graph_path}")
                    continue
            ...
            if G is None:
                G = H_doc
                with open(integrated_path, "wb") as handle:
